...
import * as LucideReact from "https://esm.sh/lucide-react@0.344.0";
...
import { 
  ...
  limit, orderBy, getDocs, addDoc, serverTimestamp
...
// Helper desacoplado para detectar Easter Eggs Bíblicos
const checkEasterEgg = (fijos, tickets, profesorSeQueda) => {
  if (!profesorSeQueda) return null;
  const f = Number(fijos) || 0;
...
export default function App() {
  ...
  useEffect(() => {
    const handleOnline = () => {
      setIsOnline(true);
      showToast("Conexión restablecida con éxito.", "success");
    };
    const handleOffline = () => {
      setIsOnline(false);
      showToast("Sin conexión a Internet. Modo local activo.", "warning");
    };
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        ...
        signInAnonymously(auth).catch((err) => {
          console.warn("Auth transparente:", err.code);
        });
      }
    });
    return () => unsubscribe();
  ...
  // Escuchar Firestore en tiempo real para la fecha seleccionada con limpieza atómica
  useEffect(() => {
    setLoadingData(true);
    setDataError(null);
    setRegistros([]); // Limpieza inmediata antes de consultar la nueva fecha
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setLoadingData(false);
      setDataError("Error al cargar los datos de Firestore. Comprueba la conexión.");
      showToast("Error al sincronizar con el servidor.", "error");
    });
    return () => unsubscribe();
  }, [selectedDate, view, db]);
        ...
        <div className="bg-slate-900 dark:bg-slate-950 text-slate-300 text-[10px] py-1 px-4 flex justify-between items-center gap-4">
          ...
          <div className="flex gap-4 items-center font-bold">
            <a 
              ...
              title="Abrir Gestión de Incidencias en el Comedor"
            >
              <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />
              <span>Incidencias Comedor</span>
...
function TeacherView({ db, user, registrosHoy, appSettings, showToast }) {
  ...
  const enableEditMode = () => {
    ...
    setAttendance(prev => {
      ...
      (rosterAlumnos || []).forEach(s => {
        const found = (yaRegistrado.especiales || []).find(e => e.rosterId === s.id);
        syncAtt[s.id] = {
          nombre: s.nombre,
          nota: s.nota,
          dietaBlanda: s.dietaBlanda,
  ...
  const especialesCount = useMemo(() => {
    const rosterCount = (rosterAlumnos || []).filter(s => {
      const att = attendance[s.id];
      if (!att) return s.tipoHabitual === "fijo";
      return att.option !== "falta" && att.asiste !== false;
    }).length;
    return rosterCount + (manualEspeciales || []).length;
  }, [rosterAlumnos, attendance, manualEspeciales]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const documentData = {
        fecha: targetDate, 
        timestamp: Date.now(), 
        etapa: formData.etapa || "Primaria", 
        curso: formData.curso || "1º", 
        letra: formData.letra || "A", 
        fijos: Number(formData.fijos) || 0, 
        tickets: Number(formData.tickets) || 0, 
        total: currentTotal,
        totalPlatos: currentTotal,
        // Modelo canónico estructurado:
        serviceType: esExcursion ? "picnic" : "dining",
        absentSpecialStudents: rosterAbsentes,
        generalObservation: observacionesTexto,
        teacherName: formData.profesorSeQueda ? (formData.profesorNombre || "").trim() : null,
        teacherEats: Boolean(formData.profesorSeQueda),
        // Campos de compatibilidad operativa:
        ...
        registradoPor: (user && user.uid) ? user.uid : "profesor_aula",
        esExcursion: Boolean(esExcursion)
      };
      ...
      // Registrar evento en auditoría de forma segura
      try {
        await logAuditEvent({
          userId: (user && user.uid) ? user.uid : "profesor_aula",
          userName: (user && (user.displayName || user.email)) ? (user.displayName || user.email) : "Docente",
          userRole: (user && user.rol) ? user.rol : "teacher",
        ...
        console.warn("Audit non-critical error:", auditErr);
      }
      ...
      const easterEggInfo = checkEasterEgg(formData.fijos, formData.tickets, formData.profesorSeQueda);
          ...
          <div className="space-y-6 animate-slide-right">
            ...
            {yaRegistrado && !isEditing ? (
              ...
              <>
                ...
                {/* SECCIÓN 1: ALÉRGICOS AUSENTES HOY */}
                <div className="bg-orange-50/20 dark:bg-orange-950/5 p-5 rounded-2xl border border-orange-200/40 dark:border-orange-900/30 space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="font-bold text-orange-850 dark:text-orange-400 flex items-center gap-2 text-xs uppercase tracking-wide">
                      <UserX className="w-4 h-4 text-orange-600" /> Alumnos Alérgicos Ausentes Hoy (No preparar ración)
                    </h3>
                    <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-orange-100 dark:bg-orange-955 text-orange-700 dark:text-orange-300">
                      {rosterAbsentes.length} ausentes
                    </span>
                  </div>
                  ...
                  {rosterAbsentes.length > 0 ? (
                    <div className="bg-white dark:bg-slate-900 border border-orange-200 dark:border-orange-900/40 rounded-xl p-3 text-xs font-semibold text-orange-800 dark:text-orange-300 flex items-start gap-2 animate-scale-up">
                      ...
                      <div>
                        <span className="block font-bold">Faltan hoy (no se preparará menú especial):</span>
                    ...
                    <div className="text-xs text-slate-500 dark:text-slate-400 italic bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200/60 dark:border-slate-800">
                      Todos los alumnos alérgicos habituales de esta clase están marcados como presentes.
                    </div>
                  )}
                </div>

                {/* SECCIÓN 2: OBSERVACIONES GENERALES E INCIDENCIAS */}
                <div className="bg-blue-50/20 dark:bg-blue-950/5 p-5 rounded-2xl border border-blue-200/40 dark:border-blue-900/30 space-y-2.5">
                  <div className="flex justify-between items-center">
                    <h3 className="font-bold text-blue-900 dark:text-blue-300 flex items-center gap-2 text-xs uppercase tracking-wide">
                      <Info className="w-4 h-4 text-blue-600" /> Observaciones del Día e Incidencias (Informativo)
                    </h3>
                    <a 
                      href="https://comedorcsb-incidencias.vercel.app/" 
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="text-[10px] font-bold text-amber-700 dark:text-amber-400 bg-amber-100/70 dark:bg-amber-950/60 hover:bg-amber-200/70 px-2 py-0.5 rounded-md transition-colors flex items-center gap-1 shadow-sm"
                      title="Abrir portal de Incidencias en nueva pestaña"
                    >
                  ...
                  <textarea 
                    className="w-full p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 text-xs focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-950 outline-none text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-900 transition-shadow"
                    placeholder="Escribe aquí observaciones generales para cocina (Ej: Salen 15 minutos antes por actividad escolar, recogida en comedor...)"
                    rows={2}
                    value={manualAusencias}
                    onChange={(e) => setManualAusencias(e.target.value)}
                    aria-label="Observaciones generales para cocina"
                  />
                </div>
...
function AdminView({ registros, selectedDate, setSelectedDate, loading, dataError, onRetry, appSettings, showToast, db }) {
  ...
  // Buscador en tiempo real para Cocina y Dirección
  const searchResults = useMemo(() => {
    ...
    registros.forEach(r => {
      const classStr = `${r.etapa} ${r.curso} ${r.letra}`.toLowerCase();
      const teacherStr = (r.profesorNombre || "").toLowerCase();
      const ausenciasStr = (r.ausencias || "").toLowerCase();

      (r.especiales || []).forEach(e => {
        if (e.nombre.toLowerCase().includes(q) || (e.nota && e.nota.toLowerCase().includes(q))) {
          matches.push({
            type: "especial",
            nombre: e.nombre,
            clase: `${r.curso} ${r.letra}`,
            etapa: r.etapa,
            nota: e.nota || (e.dietaBlanda ? "Dieta Blanda" : "Dieta Especial"),
            dietaBlanda: e.dietaBlanda,
      ...
      if (classStr.includes(q) || teacherStr.includes(q) || ausenciasStr.includes(q)) {
        matches.push({
          type: "clase",
          clase: `${r.curso} ${r.letra}`,
          etapa: r.etapa,
          total: (Number(r.fijos) || 0) + (Number(r.tickets) || 0),
  ...
  return (
    <div className="space-y-4 animate-fade-in print:space-y-6">
         ...
         <div className="flex flex-wrap gap-2 items-center w-full md:w-auto justify-end">
            {activeTab === "daily" && (
              <div className="relative flex-1 md:w-64">
                <input 
                  type="text" 
                  placeholder="🔍 Buscar alumno, alergia o clase..." 
                  value={searchQuery} 
                  onChange={e => setSearchQuery(e.target.value)} 
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-3 pr-7 py-1.5 text-xs font-semibold outline-none focus:border-blue-500 transition-all text-slate-800 dark:text-slate-100 placeholder-slate-400"
                />
                {searchQuery && (
                  <button 
                    onClick={() => setSearchQuery("")} 
                    className="absolute right-2.5 top-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 text-xs font-bold"
                  >
                    ✕
                  </button>
                )}
      ...
      {/* Panel de Resultados de Búsqueda Instantánea */}
      {activeTab === "daily" && searchQuery.trim().length > 0 && (
         <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 border-2 border-blue-500 shadow-md animate-fade-in print:hidden">
           <div className="flex justify-between items-center mb-3">
             <h4 className="font-bold text-slate-800 dark:text-slate-100 text-xs flex gap-2 items-center uppercase tracking-wider">
               🔍 Resultados de Búsqueda para: <span className="text-blue-600 dark:text-blue-400">"{searchQuery}"</span>
             </h4>
             <span className="text-[10px] bg-blue-100 dark:bg-blue-950 text-blue-800 dark:text-blue-300 px-2 py-0.5 rounded-full font-extrabold">
               {searchResults.length} coincidencias
             </span>
           </div>
           
           {searchResults.length === 0 ? (
             <div className="text-xs text-slate-400 dark:text-slate-500 italic py-3 text-center">
               No se encontraron alumnos ni clases que coincidan con la búsqueda.
             </div>
           ) : (
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-60 overflow-y-auto pr-1">
               {searchResults.map((res, i) => (
                 <div key={i} className="p-2.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200/60 dark:border-slate-700/60 flex flex-col gap-1 text-xs">
                   <div className="flex justify-between font-bold text-slate-800 dark:text-slate-200">
                     <span className="text-blue-700 dark:text-blue-400">{res.nombre || `Aula ${res.clase}`}</span>
                     <span className="text-[10px] bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 px-1.5 py-0.5 rounded font-extrabold">
                       {res.clase} ({res.etapa})
                     </span>
                   </div>
                   {res.nota && (
                     <span className="text-[10.5px] text-slate-600 dark:text-slate-300 font-semibold italic">
                       {res.nota}
                     </span>
                   )}
                   {res.profesor && (
                     <span className="text-[10px] text-slate-500 dark:text-slate-400">
                       Profesor/a: <strong>{res.profesor}</strong> ({res.total} comensales)
                     </span>
                   )}
                 </div>
               ))}
      ...
      {activeTab === "monthly" ? (
        ...
        <>
          {dataError && (
            <div className="bg-red-50 dark:bg-red-950/40 border-2 border-red-500/50 rounded-2xl p-5 text-center space-y-3 animate-shake">
              <div className="flex items-center justify-center gap-2 text-red-700 dark:text-red-300 font-bold text-sm">
                <AlertCircle className="w-5 h-5 text-red-600" />
                <span>{dataError}</span>
              </div>
              <p className="text-xs text-red-600 dark:text-red-400">No se pudieron recuperar los registros del día {selectedDate}.</p>
              {onRetry && (
                <button 
                  type="button" 
                  onClick={onRetry} 
                  className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all shadow-md active:scale-95"
                >
                  Reintentar Carga
                </button>
              )}
          ...
          {loading && (
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-10 text-center shadow-sm flex flex-col items-center justify-center gap-3 animate-fade-in">
              <RefreshCw className="w-8 h-8 text-blue-600 animate-spin" />
              <span className="font-bold text-slate-700 dark:text-slate-200 text-sm">Cargando datos del día {selectedDate}...</span>
            </div>
          )}
          ...
          {!loading && !dataError && registros.length === 0 && (
            <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 rounded-3xl p-10 text-center shadow-sm flex flex-col items-center justify-center gap-3 animate-fade-in">
              ...
              <h4 className="font-bold text-slate-800 dark:text-slate-100 text-base">No hay registros para el día {selectedDate}</h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-md">No se ha registrado ninguna asistencia de comedor ni solicitud de picnic en esta fecha. Los totales permanecen a cero.</p>
            </div>
          )}
